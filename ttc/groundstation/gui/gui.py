...
px.set_mapbox_access_token('pk.eyJ1IjoiYWRhbWhhbW1vIiwiYSI6ImNqemM3YWN4cTAzMGwzY3FtMTJwNWsxMmMifQ.--bZnHXt1i_ajrFnpdlwOQ')
...
app.layout = html.Div([
    html.H1("NICE"),
    html.Div([
        html.Div([dcc.Input(type="datetime-local", step="1", id="starttime"),dcc.Input(type="datetime-local", step="1", id="endtime")], style={'display': 'flex', 'flex-direction': 'row', "margin-bottom": "10px"}),
        dcc.Graph(id='map')
        ], style={"margin-bottom": "10px"}),
    html.Div([
        html.Div([
            dcc.Dropdown(list(WOD_OPTIONS.keys()), "Battery Voltage", id='topleft-input'),
            dcc.Graph(id='topleft-chart', style={'height': '35vh'})
        ], className="chart", style={'width': '100%'}),
        html.Div([
            dcc.Dropdown(list(WOD_OPTIONS.keys()), "Battery Current", id='topright-input'),
            dcc.Graph(id='topright-chart', style={'height': '35vh'})
        ], className="chart", style={'width': '100%'})
    ], className="row", style={'display': 'flex', 'flex-direction': 'row', 'fontSize': '16px'}),
    html.Div([
        html.Div([
            dcc.Dropdown(list(WOD_OPTIONS.keys()), "5V Current", id='bottomleft-input'),
            dcc.Graph(id='bottomleft-chart', style={'height': '35vh'})
        ], className="chart", style={'width': '100%'}),
        html.Div([
            dcc.Dropdown(list(WOD_OPTIONS.keys()), "Battery Temperature", id='bottomright-input'),
...
def new_graph(field, start_time, end_time):
    ...
    # i swear to god
    try :
        fig = px.line(pd.DataFrame(data, columns=[xlabel,ylabel]), x=xlabel, y=ylabel)
    except Exception :
        fig = px.line(pd.DataFrame(data, columns=[xlabel,ylabel]), x=xlabel, y=ylabel)
    fig.update_layout(font={'size':10}, margin={'l':0,'r':0,'t':0,'b':0}, yaxis_range=data_info[2])
    return fig
...
def update_map(start_time, end_time):
    ...
    try:
        fig = px.scatter_mapbox(data, lat="latitude", lon="longitude", color="sensor reading", hover_name="timestamp", size="altitude", zoom=1, opacity=0.05, mapbox_style='satellite', size_max=10)
    except Exception:
        fig = px.scatter_mapbox(data, lat="latitude", lon="longitude", color="sensor reading", hover_name="timestamp", size="altitude", zoom=1, opacity=0.05, mapbox_style='satellite', size_max=10)
