...
class DB:
    ...
    def get_wod(self, col, start_time, end_time):
        ...
        start = (datetime.datetime.strptime(start_time,'%Y-%m-%dT%H:%M:%S') - EPOCH_TIME).total_seconds() if start_time is not None else 0
        end = (datetime.datetime.strptime(end_time,'%Y-%m-%dT%H:%M:%S') - EPOCH_TIME).total_seconds() if end_time is not None else 1000000000000000
        cursor.execute(f"""SELECT offsetTime, `{col}` FROM wod WHERE offsetTime BETWEEN :start AND :end;""", {'start': start, 'end': end})
    ...
    def get_science(self, start_time, end_time):
        ...
        cursor.execute(f"SELECT offsetTime, latitude, longitude, 250000+altitude AS altitude, reading FROM science WHERE offsetTime BETWEEN :start AND :end;""", {'start': start, 'end': end})
        data = cursor.fetchall()
        cursor.close()
