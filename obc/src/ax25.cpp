...
void printListHex(unsigned char* l, int n) {
    for (int i = 0; i < n; i++) std::cout << std::hex << int(l[i]) << ' ';
    std::cout << '\n';
}
...
ByteArray* stuffBits(unsigned char* bytes, int nbytes) {
    ...
    unsigned char* newBytes =
        new unsigned char[2 * nbytes]();  // oversized array
    ...
    for (int byteCounter = 1; byteCounter < nbytes; byteCounter++) {
        for (int bitCounter = 0; bitCounter < 8; bitCounter++) {
            ...
            newBitCounter %= 8;
            if (newBitCounter == 0) {
                newBytes[newByteCounter] =
                    swapByteOrder(newBytes[newByteCounter]);
                newByteCounter++;
            }
    ...
    ByteArray* r = new ByteArray{newByteCounter + 1, newPayload};
    return r;
}
...
Message* decode(ByteArray* frame, int receiveState) {
    ...
    if (iFrameCheck == 0) {
        ...
        if (receiveSequence != receiveState) {
            // std::cout<<"[WARNING] Receive state does not match expected. Did
            // we miss a packet??\n";
        }
    ...
    Message* m = new Message{
        srcaddr,      destaddr,        dataType, commandResponse, controlType,
        sendSequence, receiveSequence, npayload, payload};
...
Message* searchForMessage(unsigned char* stream, int nstream,
                                int receiveState) {
    // We will need to implement something like this that is constantly
    // listening to radio transmissions. It finds the data between two `0x7e`
    // start/end flags In reality this will probably be a stream of data not
    // just a long string
    // TODO: interface with transceiver module
    unsigned char flag = 0x7e;
    unsigned char window =
        0b00000000;  // moving byte of data throughout the stream
    int maxFrameLength =
        1 + 14 + 2 + 1 + 255 + 4 + 1;  // max length of payload + overhead
...
// int main() {
//     // This is a sample use case, showing how the interface with the
//     functions
//     // should be developed
...
//     // Simulate some noise
//     std::cout<<"\nGenerating 1mio bits of random noise and searching for
//     packets\n"; unsigned char noisyMessage[1000000];
//     // TODO: move to backend of message once decoding is validated
//     memcpy( & noisyMessage[800000], encodedMsg -> bytes, encodedMsg ->
//     nbytes); Message * retrievedMsg = searchForMessage(noisyMessage, 1000000,
//     receiveState); if (retrievedMsg == NULL) {
...
//     << '\n'; std::cout<<"Command(1)/Response(0) Type : "<<
//     int(retrievedMsg->commandResponse) << '\n'; std::cout<<"Control Type
//     (0=Info, 2=Unnumbered): "<< int(retrievedMsg->controlType) << '\n';
//     std::cout<<"Sequence Nos (Send, Receive): "<<
//     int(retrievedMsg->sendSequence) << ',' <<
//     int(retrievedMsg->receiveSequence) << '\n'; std::cout<<"PAYLOAD: ";
...
//     void ByteArray_del(ByteArray * b) {
//         delete b -> bytes;
//         delete b;
//     }
...
//     ByteArray * _encode(Message * message) {
//         return encode(message);
//     }

//     Message * _searchForMessage(unsigned char * stream, int nstream, int
//     receiveState) {
//         return searchForMessage(stream, nstream, receiveState);
//     }

//     int ByteArray_getnbytes(ByteArray * b) {
//         return b -> nbytes;
//     }
//     unsigned char * ByteArray_getbytes(ByteArray * b) {
//         return b -> bytes;
//     }
